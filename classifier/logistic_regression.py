...
# train on nips stage 1 data, test on stage1_test data using logistic regression
def lr_stage1_test_valid(train_x, train_y):
    # preprocessing
    scaler = MinMaxScaler()
    ...
    print('Training logistic regression on 3321 training set...')
    clf.fit(samples, labels)
    train_score = metrics.log_loss(labels, clf.predict_proba(samples))
    print('train_score =', train_score)

    print('Testing logistic regression on 368 validation set...')
...
# n fold cross validation using logistic regression
def lr_nfold_cv(k, feat_name, x_train, x_valid, y_train, y_valid, train_index, test_index, test_samples, nfold=5):
    clf = LogisticRegression(penalty='l1', max_iter=1000)

    print('Training logistic regression on ' + str(nfold) + ' fold cross validation...')
    clf.fit(x_train, y_train)
    score = metrics.log_loss(y_valid, clf.predict_proba(x_valid))
    print('logloss on %d fold: ' % (k + 1))
    print(score)

    print('Testing logistic regression on ' + str(nfold) + ' fold cross validation...')
    ...
    try:
        kfold_submission1.to_csv('data/' + str(nfold) + 'fold_cv/train.lr.' + feat_name + '.fold_' + str(k) + '.csv', index=False)
    ...
    try:
        kfold_submission2.to_csv('data/' + str(nfold) + 'fold_cv/valid.lr.' + feat_name + '.fold_' + str(k) + '.csv', index=False)
    except FileNotFoundError:
        print(feat_name)
        kfold_submission2.to_csv('data/' + str(nfold) + 'fold_cv/valid.lr.feat_name.fold_' + str(k) + '.csv', index=False)
